...
function getBitcoinData(
  ...
  return graphqlClientWithConstantBase
    .request(MinerDataDocument, {
      limit: limit,
      offset: offset,
      from: from,
      till: till,
    ...
    <main className={styles.main}>
      <div className={styles.tableContainer}>
        <h1 style={{ textAlign: "center", color: "black" }}>
          Miner and Address statistics
        </h1>

        <div>
          <label style={{ color: "black" }} htmlFor="fromDate">
            From:{" "}
          </label>
          <input
            type="date"
          ...
          <label style={{ color: "black" }} htmlFor="tillDate">
            {" "}
            To:{" "}
          </label>
          <input
          ...
          <tbody>
            {bitcoinData.map((output, index) => (
              <tr key={index}>
                <td>
                  <Link
                    href={`/minerdetails/addressdetails?address=${
                      output.outputAddress.address
                    }&from=${fromDate.toISOString().split("T")[0]}&till=${
