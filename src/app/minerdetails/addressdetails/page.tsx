...
const AddressDetails = () => {
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.title}>Address Details for <span className={styles.highlight}>{searchParams.get('address')}</span></h2>
      <table className={styles.table}>
        ...
        <tbody>
          <tr><td>Inputs Count</td><td>{details?.bitquery.bitcoin.inputs[0]?.count || 'N/A'}</td></tr>
          <tr><td>Outputs Count</td><td>{details?.bitquery.bitcoin.outputs[0]?.count || 'N/A'}</td></tr>
          <tr><td>Earliest Transaction Date</td><td>{details?.bitquery.bitcoin.inputs[0]?.minimum || 'N/A'}</td></tr>
          <tr><td>Latest Transaction Date</td><td>{details?.bitquery.bitcoin.outputs[0]?.maximum || 'N/A'}</td></tr>
          <tr><td>Total Received (BTC)</td><td>{details?.bitquery.bitcoin.outputs[0]?.value.toFixed(8) || 'N/A'}</td></tr>
          <tr><td>Total Spent (BTC)</td><td>{details?.bitquery.bitcoin.inputs[0]?.value.toFixed(8) || 'N/A'}</td></tr>
          <tr><td>Balance (BTC)</td><td>{balance.toFixed(8)}</td></tr>
        </tbody>
      </table>
    </div>
  );
