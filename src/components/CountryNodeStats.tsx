...
const CountryNodeStatsComponent: React.FC<CountryNodeStatsProps> = ({
  mempoolCountryNodes,
}) => {
  ChartJS.register(ArcElement, Tooltip, Legend, ChartDataLabels);

  const modifiedData = mempoolCountryNodes.mempool.countryNodeStats?.map(
    (item) => ({
      ...item,
      capacity:
        item.capacity === null
          ? 0
          : parseFloat(item.capacity) >= 100000000
          ? (parseFloat(item.capacity) / 100000000).toFixed(2)
  ...
  const pieData = {
    labels: countryNodeStats!
      .slice(0, 14)
      .map((data) => data.name.en)
      .concat("Others"),
    datasets: [
      {
        data: countryNodeStats!
          .slice(0, 14)
          .map((data) => data.count)
          .concat(
            countryNodeStats!
              ...
              {countryNodeStats!.map((country, index) => (
                <tr key={country.iso}>
                  ...
                  <td className="capacity">
                    {country.capacity}{" "}
                    <span className="unit">{country.unit}</span>
                  </td>
                </tr>
              ))}
            </tbody>
