...
const Ranking: React.FC = () => {
  ...
  const getRankingInfo = async () => {
    try {
      ...
      setClearnetCapacity(
        Number.parseFloat((data.clearnetCapacity / 100000000).toFixed(2))
      );
      setTorCapacity(
        Number.parseFloat((data.torCapacity / 100000000).toFixed(2))
      );
      setUnknownCapacity(
        Number.parseFloat((data.unknownCapacity / 100000000).toFixed(2))
      );
  ...
  return (
    <div className="wrapper">
      <div className="capacity">
        <div>
          <div className="capacity-name">Clearnet Capacity</div>
          <span className="capacity-number">{clearnetCapacity}</span>
          <span className="capacity-note"> BTC</span>
        </div>
        <div>
          <div className="capacity-name">Tor Capacity</div>
          <span className="capacity-number">{torCapacity}</span>
          <span className="capacity-note"> BTC</span>
        </div>
        <div>
          <div className="capacity-name">Unknown Capacity</div>
          <span className="capacity-number">{unknownCapacity}</span>
          <span className="capacity-note"> BTC</span>
        </div>
      </div>
      <ReactECharts
